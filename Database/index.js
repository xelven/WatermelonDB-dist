...
function () {
  function Database({
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      (0, _common.invariant)(actionsEnabled === true || actionsEnabled === false, 'You must pass `actionsEnabled:` key to Database constructor. It is highly recommended you pass `actionsEnabled: true` (see documentation for more details), but can pass `actionsEnabled: false` for backwards compatibility.');
    }
  ...
  _proto.batch = function batch(...records) {
    return new Promise(function ($return, $error) {
      ...
      return Promise.resolve(this.adapter.batch(batchOperations)).then(function ($await_1) {
        try {
  ...
  _proto.experimentalSubscribe = function experimentalSubscribe(tables, subscriber) {
    ...
    return function () {
      ...
      idx !== -1 && _this3._subscribers.splice(idx, 1);
    };
  };
  ...
  _proto.unsafeResetDatabase = function unsafeResetDatabase() {
    return new Promise(function ($return, $error) {
      ...
      return Promise.resolve(this.adapter.unsafeResetDatabase()).then(function ($await_2) {
        try {
