...
var unchangedRecordsForRaws = function unchangedRecordsForRaws(raws, recordCache) {
...
var recordsToMarkAsSynced = function recordsToMarkAsSynced({
  changes: changes,
...
var destroyDeletedRecords = function destroyDeletedRecords(db, {
  changes: changes
}) {
...
function markLocalChangesAsSynced(db, syncedLocalChanges) {
  ...
  return db.action(function () {
    return new Promise(function ($return, $error) {
      return Promise.resolve(Promise.all([db.batch.apply(db, _toConsumableArray((0, _rambdax.map)(_helpers.prepareMarkAsSynced, recordsToMarkAsSynced(syncedLocalChanges)))), destroyDeletedRecords(db, syncedLocalChanges)])).then(function ($await_1) {
        try {
          return $return();
        } catch ($boundEx) {
