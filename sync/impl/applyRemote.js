...
var idsForChanges = function idsForChanges({
  created: created,
  updated: updated,
  deleted: deleted
}) {
...
var findRecord = function findRecord(id, list) {
  // perf-critical
...
function validateRemoteRaw(raw) {
  ...
  (0, _common.invariant)(raw && typeof raw === 'object' && 'id' in raw && !('_status' in raw || '_changed' in raw), "[Sync] Invalid raw record supplied to Sync. Records must be objects, must have an 'id' field, and must NOT have a '_status' or '_changed' fields");
}
...
function prepareApplyRemoteChangesToCollection(collection, recordsToApply, sendCreatedAsUpdated, log) {
  ...
  updated.forEach(function (raw) {
    ...
    if (currentRecord) {
      recordsToBatch.push((0, _helpers.prepareUpdateFromRaw)(currentRecord, raw, log));
    } else if (locallyDeletedIds.includes(raw.id)) {// Nothing to do, record was locally deleted, deletion will be pushed later
    } else {
...
var getAllRecordsToApply = function getAllRecordsToApply(db, remoteChanges) {
  return (0, _rambdax.piped)(remoteChanges, (0, _rambdax.map)(function (changes, tableName) {
    ...
    if (!collection) {
      return Promise.reject("You are trying to sync a collection named ".concat(tableName, ", but currently this collection does not exist.") + "Have you remembered to add it to your Database constructor's modelClasses property?");
    }
...
var destroyAllDeletedRecords = function destroyAllDeletedRecords(db, recordsToApply) {
...
var prepareApplyAllRemoteChanges = function prepareApplyAllRemoteChanges(db, recordsToApply, sendCreatedAsUpdated, log) {
...
var unsafeBatchesWithRecordsToApply = function unsafeBatchesWithRecordsToApply(db, recordsToApply, sendCreatedAsUpdated, log) {
  return (0, _rambdax.piped)(recordsToApply, (0, _rambdax.map)(function (records, tableName) {
